...
class Solution:
    ...
    def exist(self, board: List[List[str]], word: str) -> bool:
        ...
        for i in range(m):
            for j in range(n):
                if self.search(board, word, 0, i, j, marked, m, n):
                    return True
        return False

    def search(self, board, word, index, start_x, start_y, marked, m, n):
        # 先写递归终止条件
        if index == len(word) - 1:
        ...
        if board[start_x][start_y] == word[index]:
            ...
            for direction in self.directions:
                ...
                if 0 <= new_x < m and 0 <= new_y < n and not marked[new_x][new_y] and \
                    self.search(board, word, index + 1, new_x, new_y, marked, m, n):
                    return True
